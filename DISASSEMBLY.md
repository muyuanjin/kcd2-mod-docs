...
Retail DLL snapshots in this repo: `DLL/WHGame_release_1_5.dll` (current game, `release_1_5` / `wh_sys_version` 1.5.5) and older `WHGame_V1.2.x.dll` builds for comparison.

# Verified Offsets (Game Version V1.2.2)

> **Note:** The list below was verified against **V1.2.2**. Current retail is **release_1_5 (1.5.5)**. Prefer signature scanning (above) over hard-coded RVAs after updates. Re-validate vtable indices against `DLL/WHGame_release_1_5.dll` before shipping ASI hooks.
...
本仓库 DLL 快照：`DLL/WHGame_release_1_5.dll`（当前零售版 `release_1_5` / `wh_sys_version` 1.5.5），以及旧版 `WHGame_V1.2.x.dll` 供对照。

# 已验证的偏移(游戏版本V1.2.2)

> **注意：** 下列偏移在 **V1.2.2** 上验证。当前零售版为 **release_1_5 (1.5.5)**。游戏更新后请优先用上文签名扫描，不要直接沿用硬编码 RVA；ASI hook 前请对照 `DLL/WHGame_release_1_5.dll` 重新校验虚表序号。
